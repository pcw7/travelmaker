	...
	<div align="center">
		<ul class="pagination">
			...
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<li><a href="myBoard.do?id=${sessionScope.id}&pageNum=1"><img alt="처음으로" src="${path }/resources/images/doubleleft.png"></a></li>
				<li><a href="myBoard.do?id=${sessionScope.id}&pageNum=${pb.startPage - 1 }"><img alt="이전" src="${path }/resources/images/back.png"></a></li>			
			</c:if>
			<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage}">
				<c:if test="${pb.currentPage == i }">
					<button onclick="location.href='myBoard.do?id=${sessionScope.id}&pageNum=${i }'" class="btn4">${i}</button>
				</c:if>
				<c:if test="${pb.currentPage != i }">
					<button onclick="location.href='myBoard.do?id=${sessionScope.id}&pageNum=${i }'" class="btn4">${i}</button>
				</c:if>
			...
			<c:if test="${pb.endPage < pb.totalPage }">
				<li><a href="myBoard.do?id=${sessionScope.id}&pageNum=${pb.endPage + 1 }"><img alt="다음" src="${path }/resources/images/forward.png"></a></li>
				<li><a href="myBoard.do?id=${sessionScope.id}&pageNum=${pb.totalPage }"><img alt="마지막으로" src="${path }/resources/images/doubleright.png"></a></li>
			</c:if>
		</ul>
	</div>
</div>
